...
export default function CommunityArticle({ post, posts }: { post: CommunityPost; posts: CommunityPost[] }) {
  ...
  const figure = (image: EditorialImage, className: string, loading: "eager" | "lazy") => <figure className={className}>
    ...
    <figcaption><span>{image.caption ?? (image.credit === "author" ? "ẢNH TRẢI NGHIỆM THỰC TẾ" : "MINH HỌA TẠO BẰNG AI")}</span><span>{image.credit === "author" ? "ẢNH: KIM" : "BẮC NINH NOTE"}</span></figcaption>
  </figure>;
  ...
  return <main className="story-page">
    ...
    <article className="story">
      ...
      <div className="story-meta"><Link className="story-author" href="/gioi-thieu">{(post.author ?? "BẮC NINH NOTE").toUpperCase()}</Link> · ĐĂNG {publishedDate(post)}{updatedDate(post) ? ` · CẬP NHẬT ${updatedDate(post)}` : ""} · {post.readTime.toUpperCase()}</div>
      {heroImage ? figure(heroImage, "story-hero", "eager") : null}
    ...
    <footer><div className="article-footer-links"><Link href="/gioi-thieu">Giới thiệu</Link><Link href="/chinh-sach-bien-tap">Chính sách biên tập</Link><Link href="/quyen-rieng-tu">Quyền riêng tư</Link><Link href="/lien-he">Liên hệ</Link></div><div className="copyright">© 2026 BẮC NINH NOTE <span>VIETNAM · KOREA</span></div></footer>
  </main>;
}
