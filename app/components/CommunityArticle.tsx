...
export default function CommunityArticle({ post, posts }: { post: CommunityPost; posts: CommunityPost[] }) {
  ...
  const figure = (image: EditorialImage, className: string, loading: "eager" | "lazy") => <figure className={className}>
    ...
    <figcaption><span>MINH HỌA TẠO BẰNG AI</span><span>DABANGZZ</span></figcaption>
  </figure>;
    ...
    <article className="story">
      ...
      <div className="story-body">
        {post.sections.map((section, index) => <section className="story-section" key={section.heading}>
          ...
          {index === 1 && <div className="inline-visual"><b>{String(index + 1).padStart(2, "0")}</b><span>{post.kind === "news" ? "Kiểm tra thời gian, địa điểm và nguồn gốc trước khi chia sẻ thông tin." : "Dừng lại một chút để nhìn câu chuyện từ phía của người khác."}</span></div>}
      ...
      {post.sourceLinks?.length ? <aside className="source-panel" aria-labelledby="source-title">
        <div><span>NGUỒN CÔNG KHAI</span><h2 id="source-title">Nguồn chính thức & tài liệu tham khảo</h2></div>
        ...
        <p>Dabangzz viết bài giải thích độc lập, không sao chép nguyên văn. Hãy mở nguồn gốc để xem bản tin và thời điểm cập nhật đầy đủ.</p>
      </aside> : null}
      <div className="source-note"><strong>Ghi chú biên tập</strong><span>{post.kind === "news" ? "Dữ kiện được tách khỏi phần giải thích và dẫn tới nguồn công khai. Ảnh là minh họa AI, không phải ảnh hiện trường. Sai sót quan trọng sẽ được sửa và ghi thời điểm cập nhật." : "Nội dung được Dabangzz xây dựng từ các chủ đề thảo luận công khai, loại bỏ thông tin nhận dạng và viết lại hoàn toàn cho độc giả Việt Nam. Các tình huống tổng hợp không đại diện cho mọi người Hàn Quốc."}</span></div>
    </article>
